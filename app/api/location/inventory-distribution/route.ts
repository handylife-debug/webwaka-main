...
export const GET = withAuth(async function(request: NextRequest, context: any) {
  try {
    ...
    const validation = validateQueryParams(searchParams, 'inventory-distribution');
    if (!validation.success) {
      return NextResponse.json(validation.error, { status: 400 });
    }

    const { productFilter } = validation.data;

    console.log(`✅ Inventory distribution request: User ${context.user.id} for tenant ${tenantId}${productFilter ? ` filter: ${productFilter}` : ''}`);
